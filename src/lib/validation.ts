...
export function validateEvent(raw:unknown):EventInput {
 ...
 if(rawTeams!==undefined&&rawTeams!==null){
   ...
   for(const item of rawTeams){const t=record(item);const id=t.id,name=string(t.name,60);
     ...
     if(source!==undefined&&source!=='football-data'&&source!=='api-football'&&source!=='espn')throw new InputError('Choose a team from the search results.');
     ...
     seen.add(id);footballTeams.push({id,name,source:source as 'football-data'|'api-football'|'espn'|undefined});}
