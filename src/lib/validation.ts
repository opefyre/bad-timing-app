import { EventInput, EventKind, EventPreference, PreferenceOverrides } from '@/types';
...
export function validateEvent(raw:unknown):EventInput {
 ...
 return {venue:{address,lat:lat as number|undefined,lng:lng as number|undefined,timezone,countryCode,name:string(v.name,150),state:string(v.state,150),city:string(v.city,150),countryName:string(v.countryName,150),subdivisionCode:string(v.subdivisionCode,25)},dateTime,durationMinutes,eventKind,isOutdoor:r.isOutdoor,radiusKm,programmeId:programmeId as number|undefined,programmeName:string(r.programmeName,150),footballTeam:string(r.footballTeam,150),includeNews:r.includeNews===true,needsInternet:r.needsInternet===true};
}
